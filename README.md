...
* Separar la máquina de estados finitos de los elementos aritméticos y de almacenamiento simplificó el diseño de sistemas secuenciales complejos. En la arquitectura ASM del transmisor serial, mientras la FSM dirigió el flujo temporal entre estados, el datapath ejecutó el desplazamiento progresivo del registro (`shift_reg`) y el incremento ordenado del contador de bits (`bit_count`) de forma determinista para la entrega LSB primero.

* A diferencia de los circuitos puramente combinacionales donde las salidas cambian inmediatamente con las entradas, la sincronización por flancos de subida del reloj (`posedge clk`) permitió coordinar operaciones multi-ciclo. Esto fue esencial para mantener la temporización estricta de cada luz en el semáforo vehicular, la acumulación paso a paso en el acumulador y el sostén temporal por bit en el transmisor serial.
  
* Las simulaciones evidenciaron la importancia de diseñar señales de estatus como pulsos de duración exacta. Mediante el temporizador `tick_cnt`, se garantizó que cada bit permaneciera estable durante los `CLKS_PER_BIT` ciclos exigidos (80 ns), mientras que la bandera `done` se activó por un único ciclo de reloj al finalizar el proceso, asegurando una interfaz de comunicación síncrona y segura entre módulos.

* La inclusión de señales como `rst` y `cancel` demostró ser fundamental para la robustez del hardware. Se validó que una condición de interrupción en pleno cómputo devuelve la FSM al estado `IDLE`, reinicia los acumuladores y suprime la bandera `done`, asegurando que el sistema no propague datos inconsistentes.

* La simulación temporal mediante bancos de pruebas (*testbenches*) permitió rastrear el comportamiento interno ciclo a ciclo, facilitando la detección de condiciones de carrera, errores de conteo y verificación de estados internos antes de una eventual implementación en hardware físico.
